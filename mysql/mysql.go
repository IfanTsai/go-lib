...
func NewDB(readConfig, writeConfig *DBConfig, logFilename string) (*DB, error) {
	dbRead, err := dbConnect(readConfig.User, readConfig.Password, readConfig.Password, readConfig.Name, logFilename)
	if err != nil {
		return nil, err
	}

	dbWrite, err := dbConnect(writeConfig.User, writeConfig.Password, writeConfig.Password, writeConfig.Name, logFilename)
	if err != nil {
		return nil, err
	}
